...
export default function App() {
  ...
  const projects: Project[] = [
    {
      id: '1', 
      title: '대규모 클라우드 전환: \n DB 마이그레이션 및 정합성 검증',
      description: '우정정보관리원의 대규모 클라우드 마이그레이션 프로젝트에서 총 19개 데이터베이스(Oracle, MSSQL)의 이관 및 데이터 무결성 검증을 수행한 실무 프로젝트입니다.',
      longDescription: '1개 핵심 데이터베이스의 Import 작업을 직접 전담하고, 대전 국가정보자원관리원에서 이관한 나머지 18개 DB의 정합성을 검증했습니다. 할당된 스토리지 부족 문제를 해결하기 위해 사전 DB 슬림화 작업을 기획했으며, Linux 환경에서 awk 스크립트와 sqlplus를 활용해 대용량 로그 파싱 및 오브젝트 검증을 자동화하여 시스템 무결성을 확보했습니다.',
      image: p1Image, 
      ...
      troubleshooting: [
        ...
        },
        {
          ...
          result: '휴먼 에러 방지 및 빠르고 정확한 데이터 정합성 검증(테이블, 레코드 수) 완료'
        },
        {
          ...
          result: '오류 및 누락 건수 0건 달성으로 오차율 0%의 완벽한 오브젝트 이관 검증 달성'
        },
      ],
      retrospect: [
  ...
  const experiences: TimelineItem[] = [
    {
      id: '1',
      title: '데이터 이관 및 정합성 검증(ORACLE, MSSQL)',
      company: '우정정보관리원',
      location: '나주, 대한민국',
      period: '2026.01 - 현재', 
      description: [
        '대규모 클라우드 마이그레이션 프로젝트 내 총 19개 DB 대상 데이터 이관 및 정합성 검증 수행',
        '할당된 데이터베이스 공간 부족 문제를 해결하기 위한 DB 슬림화 작업 기획 및 사전 준비',
